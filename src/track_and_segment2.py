...
def process_video(args, stream=False):
    """
    Process the video for vessel tracking and segmentation.
    If `stream=True`, yields frames for real-time visualization (for Gradio).
    """
    ...
    # Line positions
    black_line_x = 1090
    blue_line_x = black_line_x - 150  # Existing blue line
    pink_line_x = black_line_x - 420  # New pink line (further left)
    tolerance = 5
    ...
    while cap.isOpened():
            ...
            for box in boxes:
                ...
                if class_id == 8 and confidence > 0.5:  # Boat class detected
                    ...
                    if blue_line_x - tolerance <= center_x <= blue_line_x + tolerance:
                        ...
                        if save_segmented_vessel(frame, xyxy, vessel_id, frame_count, output_dirs, args):
        ...
        # Draw vertical lines on the annotated frame
        cv2.line(annotated_frame, (black_line_x, 0), (black_line_x, annotated_frame.shape[0]), (0, 0, 0), 10)  # Black Line
        cv2.line(annotated_frame, (blue_line_x, 0), (blue_line_x, annotated_frame.shape[0]), (255, 0, 0), 10)  # Blue Line
        cv2.line(annotated_frame, (pink_line_x, 0), (pink_line_x, annotated_frame.shape[0]), (255, 105, 180), 10)  # Pink Line


        if stream:
            # Yield the annotated frame for streaming
...
def save_segmented_vessel(frame, box, vessel_id, frame_count, output_dirs, args):
...
def main():
    ...
    print(args)
    print(args.video_filename)

    # Use the visualization argument to control whether the frames are displayed.
    for frame in process_video(args=args, stream=args.enable_visualization):
        if args.enable_visualization:
            # Display in a CV2 window for standalone execution
            frame_np = cv2.imdecode(np.frombuffer(frame, np.uint8), cv2.IMREAD_COLOR)
            cv2.imshow("YOLO Inference", frame_np)
